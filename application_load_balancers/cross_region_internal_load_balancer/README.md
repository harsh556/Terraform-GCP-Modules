...
| `vpc_config`   | VPC Config Object    | Configuration related to the VPC network.                                         | Yes      |         |
| `protocol`     | string      | The protocol to use for the load balancer. Valid values are `"HTTP"` or `"HTTPS"`.  | Yes      |         |
| `backend_type` | optional(string) | Specifies whether the backends are instance groups or network endpoint groups. Supported values: `GCE_VM_IP_PORT`, `GCE_VM`, `SERVERLESS`, `PRIVATE_SERVICE_CONNECT`. Defaults to `GCE_VM_IP_PORT`. | No       |         |
| `urlmap_config`| optional(URL Map Object) | Configuration for the URL map, which defines how traffic is routed based on the incoming request's host and path. If not specified, a default URL map is created | No       |  `{default_service = "default"}`       |
